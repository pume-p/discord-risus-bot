...
curl -n -X DELETE https://api.heroku.com/apps/discord-risus-bot/dynos -H "Content-Type: application/json" -H "Accept: application/vnd.heroku+json; version=3" -H "Authorization: Bearer ${heroku_token}"